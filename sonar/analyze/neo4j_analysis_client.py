...
class Neo4jAnalysisClient(Neo4jClient):
    ...
    def calculate_betweenness_centrality(self, username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        with self.driver.session() as session:
            Neo4jAnalysisClient._create_named_graph(session, username, catalog_base_name, catalog_extension_name, graph_type)
            result = session.execute_read(Neo4jAnalysisClient._calculate_betweenness_centrality,  username, catalog_base_name, catalog_extension_name, graph_type)
            Neo4jAnalysisClient._remove_named_graph(session, username, catalog_base_name, catalog_extension_name)
            return result
    ...
    def _calculate_betweenness_centrality(tx,  username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        if graph_type == ('Article', 'CITES'):
            ...
            betweenness_query = """CALL gds.betweenness.stream('{username}/{catalog_base_name}/{catalog_extension_name}', {{relationshipWeightProperty: 'weight'}}) YIELD nodeId, score RETURN gds.util.asNode(nodeId) AS {node_type}, score AS betweenness_centrality_score ORDER BY score DESC""".format(username=username, catalog_base_name=catalog_base_name, catalog_extension_name=catalog_extension_name, node_type=graph_type[0])
    ...
    def calculate_eigenvector_centrality(self, username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        with self.driver.session() as session:
            Neo4jAnalysisClient._create_named_graph(session, username, catalog_base_name, catalog_extension_name, graph_type)
            result = session.execute_read(Neo4jAnalysisClient._calculate_eigenvector_centrality,  username, catalog_base_name, catalog_extension_name, graph_type)
            Neo4jAnalysisClient._remove_named_graph(session, username, catalog_base_name, catalog_extension_name)
            return result
    ...
    def _calculate_eigenvector_centrality(tx,  username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        if graph_type == ('Article', 'CITES'):
            ...
            eigenvector_query = """CALL gds.eigenvector.stream('{username}/{catalog_base_name}/{catalog_extension_name}', {{relationshipWeightProperty: 'weight'}}) YIELD nodeId, score RETURN gds.util.asNode(nodeId) AS {node_type}, score AS eigenvector_centrality_score ORDER BY score DESC""".format(username=username, catalog_base_name=catalog_base_name, catalog_extension_name=catalog_extension_name, node_type=graph_type[0])
    ...
    def calculate_page_rank(self, username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        with self.driver.session() as session:
            Neo4jAnalysisClient._create_named_graph(session, username, catalog_base_name, catalog_extension_name, graph_type)
            result = session.execute_read(Neo4jAnalysisClient._calculate_page_rank,  username, catalog_base_name, catalog_extension_name, graph_type)
            Neo4jAnalysisClient._remove_named_graph(session, username, catalog_base_name, catalog_extension_name)
            return result
    ...
    def _calculate_page_rank(tx,  username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        if graph_type == ('Article', 'CITES'):
            ...
            pagerank_query = """CALL gds.pageRank.stream('{username}/{catalog_base_name}/{catalog_extension_name}', {{relationshipWeightProperty: 'weight'}}) YIELD nodeId, score RETURN gds.util.asNode(nodeId) AS {node_type}, score AS pagerank_score ORDER BY score DESC""".format(username=username, catalog_base_name=catalog_base_name, catalog_extension_name=catalog_extension_name, node_type=graph_type[0])
    ...
    def calculate_degree_centrality(self, username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        with self.driver.session() as session:
            Neo4jAnalysisClient._create_named_graph(session, username, catalog_base_name, catalog_extension_name, graph_type)
            result = session.execute_read(Neo4jAnalysisClient._calculate_degree_centrality,  username, catalog_base_name, catalog_extension_name, graph_type)
            Neo4jAnalysisClient._remove_named_graph(session, username, catalog_base_name, catalog_extension_name)
            return result
    ...
    def _calculate_degree_centrality(tx,  username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        if graph_type == ('Article', 'CITES'):
            ...
            degree_query = """CALL gds.degree.stream('{username}/{catalog_base_name}/{catalog_extension_name}', {{relationshipWeightProperty: 'weight'}}) YIELD nodeId, score RETURN gds.util.asNode(nodeId) AS {node_type}, score AS degree_centrality_score ORDER BY score DESC""".format(username=username, catalog_base_name=catalog_base_name, catalog_extension_name=catalog_extension_name, node_type=graph_type[0])
    ...
    def calculate_article_rank(self, username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        with self.driver.session() as session:
            Neo4jAnalysisClient._create_named_graph(session, username, catalog_base_name, catalog_extension_name, graph_type)
            result = session.execute_read(Neo4jAnalysisClient._calculate_article_rank,  username, catalog_base_name, catalog_extension_name, graph_type)
            Neo4jAnalysisClient._remove_named_graph(session, username, catalog_base_name, catalog_extension_name)
            return result
    ...
    def _calculate_article_rank(tx,  username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        if graph_type == ('Article', 'CITES'):
            ...
            article_rank_query = """CALL gds.articleRank.stream('{username}/{catalog_base_name}/{catalog_extension_name}', {{relationshipWeightProperty: 'weight'}}) YIELD nodeId, score RETURN gds.util.asNode(nodeId) AS {node_type}, score AS article_rank_score ORDER BY score DESC""".format(username=username, catalog_base_name=catalog_base_name, catalog_extension_name=catalog_extension_name, node_type=graph_type[0])
    ...
    def calculate_closeness_centrality(self, username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        with self.driver.session() as session:
            Neo4jAnalysisClient._create_named_graph(session, username, catalog_base_name, catalog_extension_name, graph_type)
            result = session.execute_read(Neo4jAnalysisClient._calculate_closeness_centrality,  username, catalog_base_name, catalog_extension_name, graph_type)
            Neo4jAnalysisClient._remove_named_graph(session, username, catalog_base_name, catalog_extension_name)
            return result
    ...
    def calculate_harmonic_centrality(self, username, catalog_base_name, catalog_extension_name, graph_type):
        ...
        with self.driver.session() as session:
            Neo4jAnalysisClient._create_named_graph(session, username, catalog_base_name, catalog_extension_name, graph_type)
            result = session.execute_read(Neo4jAnalysisClient._calculate_harmonic_centrality,  username, catalog_base_name, catalog_extension_name, graph_type)
            Neo4jAnalysisClient._remove_named_graph(session, username, catalog_base_name, catalog_extension_name)
            return result
